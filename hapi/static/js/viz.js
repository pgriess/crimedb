...
requirejs(
    ...
    function(jquery, jqueryUi, L, vizUtil) {
        ...
        var getRegionData = function(map, cb) {
            jquery.getJSON('/regions', cb);
        };

        var updateMap = function(map) {
            getRegionData(map, function(regionData) {
                ...
                var url = vizUtil.buildURL(
                    '/crimes', {
                        limit: 1000000,
                        time: '[1356998400:1388534400]',
                        location: vizUtil.geoJSONToCrimeDBLocation(vizUtil.leafletBoundsToGeoJSON(bounds)),
                    }
                );
                ...
                jquery.getJSON(
                    ...
                    function (crimeData) {
                        ...
                        /* Add region line */
                        var l = L.geoJson(regionData.regions.stl, {
                            style: function (f) {
                                return {
                                    fill: false,
                                    clickable: false,
